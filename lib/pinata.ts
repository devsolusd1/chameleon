 ...
 *
 * Returned URLs point at OUR /ipfs/<cid> proxy (edge-cached, immutable)
 * so bots and wallets never hit rate-limited public gateways.
 */
...
export async function pinFile(buffer: Buffer, filename: string, mime: string): Promise<string> {
  ...
  return `${BASE_URL}/ipfs/${data.IpfsHash}`;
}
